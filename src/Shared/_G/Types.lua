...
--[[**
	ensures value is a specific class
...
do
	local cache = {}
	function t.class(className: string)
		if not cache[className] then
			cache[className] = function(value)
				if typeof(value) == "Instance" then
					if value.ClassName == value then
						return true
					else
						return false, string.format("%s expected, got %s", className, value.ClassName)
					end
				else
					return false, string.format("%s expected, got %s", className, typeof(value))
				end
			end
		end
